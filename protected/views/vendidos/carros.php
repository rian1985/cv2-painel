...
<?php } ?>
        ...
        <?php if($veiculos['destaque'] === '1'){?>
         <div class="ItensAcao"> <?php if($veiculos['tipo'] === 'Carro'){ ?><a href="#" class="Visualizar">Visualizar</a> <?php } ?>
             <?php if($veiculos['tipo'] === 'Carro'){ ?>| <a href="<?php echo $this->createUrl('carros/tiraDestaque/') . "?id=".$veiculos['id']." "; ?>" class="Destacado">Tirar destaque</a> <?php } ?>
              <?php if($veiculos['tipo'] === 'Carro'){ ?>| <a href="<?php echo $this->createUrl('carros/update/') . "?id=".$veiculos['id']." "; ?>" class="Alterar">Alterar</a> <?php } ?>
            <?php if($veiculos['tipo'] === 'Carro'){ ?>| <a href="<?php echo $this->createUrl('carros/delete/') . "?id=".$veiculos['id']." "; ?>" class="Apagar">Apagar</a> <?php } ?>

         </div>
             
   <?php } if($veiculos['destaque'] === '0') { ?>
         <div class="ItensAcao"><?php if($veiculos['tipo'] === 'Carro'){ ?><a href="#" class="Visualizar">Visualizar</a> <?php } ?>
             <?php if($veiculos['tipo'] === 'Carro'){ ?>| <a href="<?php echo $this->createUrl('carros/destaque/') . "?id=".$veiculos['id']." "; ?>" class="Destacado">Destacar</a> <?php } ?>
              <?php if($veiculos['tipo'] === 'Carro'){ ?>| <a href="<?php echo $this->createUrl('carros/update/') . "?id=".$veiculos['id']." "; ?>" class="Alterar">Alterar</a> <?php } ?>
            <?php if($veiculos['tipo'] === 'Carro'){ ?>| <a href="<?php echo $this->createUrl('carros/delete/') . "?id=".$veiculos['id']." "; ?>" class="Apagar">Apagar</a> <?php } ?>
