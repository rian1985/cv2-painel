         ...
         public function accessRules()
 {
  return array(
   ...
   array('deny',  // deny all users
	...
	{
		$carros = Yii::app()->db->createCommand("SELECT `cv2_veiculos_veiculos`.id AS `id_veiculo`,
...
WHERE cv2_veiculos_veiculos.`status` = 0 AND cv2_veiculos_veiculos.id_vendedor = ".Yii::app()->user->id_vendedor." ORDER BY cv2_veiculos_veiculos.data_cadastro DESC LIMIT 3 ;")->queryAll();

                $criteria = new CDbCriteria;
    $criteria->order = "data DESC";
    $criteria->limit = 3; 
                $mensagens = Cv2Mensagens::model()->findAll($criteria);
                
         $this->render('index', array(
            'carros' => $carros,'mensagens' => $mensagens
        ));
	}
 ...
 Yii::app()->user->setFlash('login-error', 'Usuário e/ou Senha incorreto');
  }
